<template>
  <div v-if="parseHtml" class="chat-bubble" :class="{ 'chat-menu': noPadding }">
    <div v-if="showFace" class="face">
      <svg-icon icon-class="logo" />
    </div>
    <div v-if="parseHtml" class="chat-content">
      <span v-if="showFace" class="arrow" />
      <span v-html="parseHtml" />
    </div>
  </div>
</template>
...
const index = ref(0)
// const formatContent = ref<string>("")
// const destroyEdit = ref<boolean>(false)
